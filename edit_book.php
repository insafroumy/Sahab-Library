...
$bname_err = $aname_err = $bdesc_err = $bdate_err = $catname_err = $bimg_err = $bpdf_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$input_bname = test_input($_POST["bname"]);
	if (empty($input_bname)) {
		$bname_err = "Please enter the name of the book.";
	} else {
		$bname = $input_bname;
	}

	$input_aname = test_input($_POST["aname"]);
	...
	$input_bdesc = test_input($_POST["bdesc"]);
	if (empty($input_bdesc)) {
	...
	if (empty($input_date)) {
		$bdate_err = "Please enter the date.";
	} else {
		$bdate = $input_date;
	}

	$input_catname = test_input($_POST["catname"]);
	if (empty($input_catname)) {
		$catname_err = "Please enter the category name";
	} else {
		$catname = $input_catname;
	}

	if (isset($_FILES['bpdf']['name'])&& $_FILES['bpdf']['name'] != "") {
		$bpdf = $_FILES['bpdf']['name'];
		$file_tmp = $_FILES['bpdf']['tmp_name'];
		$ext= explode(".",$bpdf);
		$pdf_name = "assets/bookFiles/".$bpdf;
		move_uploaded_file($file_tmp,$pdf_name);
	}else{
		$bpdf_err ="Please enter a pdf file";
	}

	if (isset($_FILES['bimg']['name'])&& $_FILES['bimg']['name'] != "") {
		$bimg = $_FILES['bimg']['name'];
		$file_tmp = $_FILES['bimg']['tmp_name'];
		$ext= explode(".",$bimg);
		$image_name = "assets/bookImages/".$bimg;
		move_uploaded_file($file_tmp,$image_name);
	}else{
		$bimg_err ="Please enter an image";
	}

	$eid = $_GET['cid'];

	if (
		empty($bname_err) && empty($aname_err) && empty($bdate_err) && empty($bdesc_err) &&
		empty($catname_err) && empty($bimg_err) && empty($bpdf_err)
	) {

		$stmt = $link->prepare('UPDATE book set bname=? , aname=? ,bdesc=? , bdate=? , catname=? , bimg=? , bpdf=?  WHERE bid=?');
					...
					<div id="kt_aside_menu" class="kt-aside-menu" data-ktmenu-vertical="1" data-ktmenu-scroll="1"
						...
						<ul class="kt-menu__nav">
							...
							<li class="kt-menu__item kt-menu__item--active" aria-haspopup="true"><a
									href="index_book.php" class="kt-menu__link "><span class="kt-menu__link-icon">
							...
							<li class="kt-menu__item " aria-haspopup="true"><a href="index_setting.php"
									class="kt-menu__link "><span class="kt-menu__link-icon">
								...
								<div class="kt-portlet__head">
									<div class="kt-portlet__head-label">
										...
										<i class="la la-pencil-square-o text-info fa-2x"></i>
											<span style="font-size : 23px">Edit Book</span>
										</h3>
									</div>
									...
									<div class="kt-portlet__content">
										...
										<form method="post" enctype="multipart/form-data">
											<div class="row">
												...
												$eid = $_GET['cid'];
												$ret = mysqli_query($link, "select * from book where bid='$eid'");
												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<textarea
															class="form-control "
															name="bdesc" 
															placeholder="Brief Description of Book"><?php echo $row['bdesc']; ?>
															</textarea>
														<span class="error text-danger">
															<?php echo $bdesc_err ?>
														</span>
												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<select class="form-control <?php echo (!empty($catname_err)) ? 'is-invalid' : ''; ?>"
														name="catname" >
												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<input type="file" name="bimg" value="<?php echo $row['bimg']; ?>"
															class="form-control "
												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<input type="file" name="bpdf" value="<?php echo $row['bpdf']; ?>"
															class="form-control "
															accept=".pdf" title="Upload PDF" />
														<span class="error text-danger">
															<?php echo $bpdf_err ?>
														</span>
													</div>
